...
function convertAwaitBlock(node, parent, ctx) {
    ...
    if (!node.then.skip) {
        ...
        if (node.value) {
            ctx.scriptLet.nestBlock(thenBlock, [node.value], [thenBlock], ([value]) => {
    ...
    if (!node.catch.skip) {
        ...
        const catchStart = awaitBlock.pending || awaitBlock.then
            ? node.catch.start
            : node.start;
        ...
        if (node.error) {
            ctx.scriptLet.nestBlock(catchBlock, [node.error], [catchBlock], ([error]) => {
